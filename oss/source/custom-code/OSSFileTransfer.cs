...
namespace Autodesk.Oss
{
    ...
    public class OSSFileTransfer : IOSSFileTransfer
    {
            ...
            string requestId)
        {
            ...
            var streamAsync = await _forgeService.Client.GetByteArrayAsync(contentUrl);
            _forgeService.Client.DefaultRequestHeaders.Remove("Range");
            var available = streamAsync.Length;
            await outStream.WriteAsync(streamAsync, 0, available);
        }
